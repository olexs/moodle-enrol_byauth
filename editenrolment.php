...
/**
 * byauth user enrolment edit script.
 *
 * This page allows the current user to edit a byauth user enrolment.
...
require_once("$CFG->dirroot/enrol/byauth/editenrolment_form.php"); // Forms for this page
...
// The user must be able to manage byauth enrolments within the course
require_capability("enrol/byauth:manage", get_context_instance(CONTEXT_COURSE, $course->id, MUST_EXIST));
...
$url = new moodle_url('/enrol/byauth/editenrolment.php', $returnurl->params());
...
if (!$plugin->allow_manage($instance) || $instance->enrol != 'byauth' || !($plugin instanceof enrol_byauth_plugin)) {
    print_error('erroreditenrolment', 'enrol');
}

// Get the byauth enrolment edit form
$mform = new enrol_byauth_user_enrolment_form($url, array('user'=>$user, 'course'=>$course, 'ue'=>$ue));
...
$title = get_string('editenrolment', 'enrol_byauth');
